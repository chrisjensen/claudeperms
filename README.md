...
A `PreToolUse` hook for [Claude Code](https://docs.anthropic.com/en/docs/claude-code) that adds defense-in-depth checks on top of the built-in allow/deny lists. Single file, zero dependencies. Every allowlist is a plain text or JSON file under `~/.claudeperms/` — the defaults ship in `defaults/` and you copy them into place during install.
...
2. Deploy the hook + defaults into `~/.claudeperms/`:
   ...
   Always overwrites `~/.claudeperms/permissions.mjs` (the code). Copies any missing config files from `defaults/`, but won't overwrite ones you've edited.
...
Runs the suite via `node --test test/*.test.mjs`. Zero dependencies — no `npm install` needed. Each test spawns the hook with a fresh temp `HOME` and seeds it with `defaults/*`, so the real `~/.claudeperms/` is never read.
