...
|---|---|---|
| `config.json` | General hook config. `trash.enabled` (bool) toggles the deletion deny, `trash.location` (string, supports `~/` and cwd-relative) names the trash directory referenced in deny messages and used for the "deletion inside trash" carve-out. `chainedHooks` (array, **ordered**) lists other PreToolUse hooks this one runs and digests — see the chained-hooks note below. | Trash disabled — all `rm`/`shred`/`unlink`/`truncate`/`cat /dev/null >` commands ask instead of denying. `chainedHooks` empty/missing → no chained hooks (own check only). |
...
- **Chained PreToolUse hooks.** A harness runs multiple `PreToolUse` hooks unpredictably, so claude-perms can be the **single** registered hook and run the others itself. List them in `config.json` as `chainedHooks: [{ "matcher": "Bash", "command": "…" }]` — **order is significant** (`matcher` `""`/`"*"` = all tools). Each entry is fed the tool-call JSON on stdin and follows the harness's own contract: **exit 2 + stderr = deny** (a gate), or **exit 0 with stdout JSON** carrying `hookSpecificOutput.updatedInput.command` (a rewrite) and/or `permissionDecision` (a verdict). Any other outcome fails **open** (a broken/slow hook never blocks or mutates). The pipeline runs the entries in order — **rewriters thread their rewrite forward**, gates contribute verdicts, a gate deny short-circuits — then claude-perms' **own** permission check runs **last, on the fully-rewritten command**. The final verdict is `deny > ask > allow`; on a surviving `allow` the accumulated rewrite is emitted as `updatedInput` so the harness auto-approves it. This is how rtk (a rewriter) is composed: register it as the first `chainedHooks` entry rather than a separate hook.
- **rtk wrapper.** When [rtk](https://github.com/rtk-ai/rtk) is active (registered as a `chainedHooks` rewriter, above), its hook auto-rewrites commands to `rtk <cmd>` (and `cat` to `rtk read`); `rtk proxy <cmd>` runs `<cmd>` verbatim. The read-only and jq carve-outs strip a leading `rtk`/`rtk proxy` before matching, so wrapped native commands reuse the existing `read-only-commands`/`jq-pipeline-helpers` entries. The deny/ask safety gates (`rm`, `dd`, `/proc`, truncation, sensitive files) scan the whole string, so the wrapper can't smuggle anything past them. rtk-specific read-only verbs (`read`, `gain`, …) are listed in `read-only-commands`.
